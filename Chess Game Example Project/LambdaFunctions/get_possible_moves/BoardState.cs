...
namespace AWSSDK.Examples.ChessGame
{
    ...
    public class BoardState
    {
        ...
        private BoardState(BoardState previousState, ChessMove newMove, bool lookForCheck = true)
        {
            ...
            {
                var previousPossibleMoves = previousState.GetPossibleMoves(newMove.From);
            ...
            if (newMove.KingsideCastle || newMove.QueensideCastle)
            {
                var row = TurnColor == ChessPieceColor.White ? FirstRowWhite : FirstRowBlack;
                var rookStartColumn = newMove.KingsideCastle ? KingsideRookColumn : QueensideRookColumn;
                var kingEndColumn = newMove.KingsideCastle ? KingsideCastledKingColumn : QueensideCastledKingColumn;
                var rookEndColumn = newMove.KingsideCastle ? KingsideCastledRookColumn : QueensideCastledRookColumn;
            ...
            else
            {
                ...
                if (newMove.PieceType == ChessPieceType.Pawn)
                {
                    ...
                    var pawnDoubleFromRow = TurnColor == ChessPieceColor.White ? 1 : 6;
                    var pawnDoubleToRow = TurnColor == ChessPieceColor.White ? 3 : 4;
                    var enPassantTargetTargetRow = TurnColor == ChessPieceColor.White ? 2 : 5;
        ...
        private BoardState(BoardState previousState, out bool isCheck, out bool isCheckMate)
        {
            ...
            if (isCheck)
            {
                ...
                isCheckMate = true;
                for (var row = 0; row < 8 && isCheckMate; row++)
                {
                    for (var column = 0; column < 8 && isCheckMate; column++)
                    {
        ...
        private void ParseFEN(string fen)
        {
            ...
            var sections = System.Text.RegularExpressions.Regex.Split(fen.Trim(), @"\s+");
            ...
            for (var i = 0; i < FenSectionCount; i++)
            {
        ...
        private bool KingIsCapturable()
        {
            ...
            for (var row = 0; row < 8; row++)
            {
                for (var column = 0; column < 8; column++)
                {
                    foreach (var move in PossibleMovesGrid[row, column])
                    {
        ...
        private List<ChessMove> DetermineIndefiniteDirectionalMovements(List<int[]> directions, Coordinate fromCoordinate, ChessPiece fromPiece)
        {
            ...
            foreach (var direction in directions)
            {
                ...
                while (true)
                {
                    ...
                    {
                        var potentialCoordinatePiece = GetPieceAtCoordinate(potentialCoordinate);
                        var blocked = potentialCoordinatePiece.Type != ChessPieceType.None;
                        var capture = blocked && potentialCoordinatePiece.Color != TurnColor;
        ...
        {
            var legalMoves = new HashSet<ChessMove>();
            var fromPiece = GetPieceAtCoordinate(fromCoordinate);
            ...
            switch (fromPiece.Type)
            {
                ...
                case ChessPieceType.Pawn:
                    ...
                    var pawnForwardAmountAllowed = 1;
                    if (TurnColor == ChessPieceColor.White)
                    {
                        pawnDirection = 1;
                    ...
                    while (forwardMoved <= pawnForwardAmountAllowed)
                    {
                        ...
                        if (potentialCoordinate.IsInBoardBounds() && GetPieceAtCoordinate(potentialCoordinate).Type == ChessPieceType.None)
                        {
                            ...
                            var isPromotionToQueen = (potentialCoordinate.Row == (TurnColor == ChessPieceColor.White ? 7 : 0));
                            // Check and checkmate values will only be verified after a move is chosen, to avoid the heavy processing for each move.
                    ...
                    if (potentialCoordinate.IsInBoardBounds())
                    {
                        var pieceAtPotentialCoordinate = GetPieceAtCoordinate(potentialCoordinate);
        ...
        private void ParsePosistions(string ranks, string fen)
        {
            ...
            foreach (var c in ranks)
            {
                ...
                if (CharToChessPieceType.ContainsKey(Char.ToUpper(c)))
                {
                    ...
                    var color = Char.IsUpper(c) ? ChessPieceColor.White : ChessPieceColor.Black;
                    var type = CharToChessPieceType[Char.ToUpper(c)];
        ...
        private void ParseLongAlgebraicNotation(string moveString)
        {
            ...
            var moveIsKingsideCastle = false;
            var moveIsQueensideCastle = false;
            ...
            for (var row = 7; row >= 0; row--)
            {
                for (var column = 0; column < 8; column++)
                {
                    ...
                    else
                    {
                        ...
                        var c = ChessPieceToCapitalChar[(int)(BoardGrid[row, column].Type)];
